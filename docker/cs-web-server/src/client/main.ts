...
async function fetchWithProgress(url: string) {
  ...
  while (true) {
    ...
    if (total !== null) {
      // progress.max é 100 no HTML, então precisamos escalar a fração (0-1) para 0-100
...
async function main() {
  ...
  const config = await fetch("/config").then(res => res.json()) as Awaited<{
    arguments: string[]; console: string[]; game_dir: string; libraries: {
      client: string; server: string; extras: string; menu: string; filesystem: string;
    }; dynamic_libraries: string[]; files_map: Record<string, string>;
  }>
  ...
  // Assets prontos: libera o botão Start mudando para Verde e alterando o texto
  ...
  // Aguarda o jogador digitar o nick e clicar no botão "Start Game"
  const username = await usernamePromise
  x.main()

  // Ajustes de mira
  x.Cmd_ExecuteString('cl_crosshair_size small')
  x.Cmd_ExecuteString('cl_dynamiccrosshair 0')
  ...
  // Jogo carregado por completo e conectado: some com a logo de vez
